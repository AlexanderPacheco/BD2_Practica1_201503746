...
docker run -h subscriber --name subscriber -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=Pa55w0rd!' -p 1455:1433 -d mcr.microsoft.com/mssql/server:2019-CU15-ubuntu-20.04
docker run -h subscriber1 --name subscriber1 -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=Pa55w0rd!' -p 1452:1433 -d mcr.microsoft.com/mssql/server:2019-CU15-ubuntu-20.04
docker run -h subscriber2 --name subscriber2 -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=Pa55w0rd!' -p 1453:1433 -d mcr.microsoft.com/mssql/server:2019-CU15-ubuntu-20.04
docker run -h distributor --name distributor -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=Pa55w0rd!' -p 1454:1433 -d mcr.microsoft.com/mssql/server:2019-CU15-ubuntu-20.04
```
...
-- Add the publication (this will create the snapshot agent if we wanted to use it)
-- Rename the BD name to expose to subscriber
EXEC sp_addpublication @publication = N'USAC_201503746DB2',
                       @description = N'',
                       @retention = 0,
                       @allow_push = N'true',
                       @repl_freq = N'continuous',
...
-- an article(or table) let will published
USE [USAC_201503746];
EXEC sp_addarticle @publication = N'USAC_201503746DB2', --aliasBD
                   @article = N'suppliers',
                   @source_owner = N'dbo',
                   @source_object = N'suppliers',
                   @type = N'logbased',
...
use [USAC_201503746]
exec sp_addsubscription
@publication = N'USAC_201503746DB2', --aliasBD
...
-- and add the push agent
exec sp_addpushsubscription_agent
@publication = N'USAC_201503746DB2',
@subscriber = 'subscriber',
@subscriber_db = 'USAC_201503746',
@subscriber_security_mode = 0,
@subscriber_login =  'sa',
